#!/usr/bin/env tclsh
# dataframe.tcl -- tests for the print-survivable data frame + AST.
...
set outdir [file join $here out-dataframe]
...
set ast [::convertkit::tclast::parseScript $src]
check "parses sample into nodes" [expr {[llength $ast] > 3}]
set regen [::convertkit::tclast::astToTcl $ast]
...
set tres [::convertkit::dataframe::encode -source "puts hi\n" -out $tinyframe \
    -pagewmm 140 -pagehmm 100 -cellmm 2.0 -bandmm 26 -quietmm 6 -dpi 120 \
...
set d [::convertkit::dataframe::decode $frame]
...
set d [::convertkit::dataframe::decode $shot]
...
set d [::convertkit::dataframe::decode $shot2]
...
set d [::convertkit::dataframe::decode $scan]
...
set res [::convertkit::dataframe::encode -source $big -out $bframe \
    -pagewmm 140 -pagehmm 100 -cellmm 2.0 -bandmm 26 -quietmm 6 -dpi 120 \
    -host testhost -ip 10.0.0.42 -id 8]
check "encoder flags incomplete" [expr {![dict get $res complete]}]
set d [::convertkit::dataframe::decode $bframe]
