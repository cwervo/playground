...
int main( int argc, char** argv ) {
  ...
  cout << "args [2]: " << argv[2] << endl;
  img = imread(samples::findFile( argv[1]), IMREAD_GRAYSCALE);
  templ = imread(samples::findFile( argv[2]), IMREAD_GRAYSCALE);
  if(argc > 3) {
    use_mask = true;
    mask = imread( argv[3]);
  }
  cout << "img: " << img.size() << endl;
  cout << "templ: " << templ.size() << endl;
...
//   imshow( result_window, result );
  imwrite("/home/folk/folk-images/result2.jpg", result);
  return;
}