...
void main(void) {
    ...
    // Apply a blue tint - stronger on highlights
    float luminance = dot(brightenedColor, vec3(0.2126, 0.7152, 0.0722));
    vec3 blueTint = vec3(0.6, 0.8, 1.0); // Cool blue
    // Mix based on luminance, more blue in brighter areas
...
void main(void) {
    ...
    // 1. Faint white/yellow cast
    vec3 warmCastColor = vec3(1.0, 0.98, 0.85); // Light yellowish white
    vec3 colorWithCast = mix(originalColor.rgb, warmCastColor, 0.12);

    // 2. Simple fake lens flare (a bright spot)
    // Flare position (e.g., top-leftish, normalized 0-1)
    vec2 flarePosition = vec2(0.2, 0.8); 
    // Distance from current pixel to flare center
    float distToFlare = distance(vTexCoord, flarePosition);
    
    // Flare intensity - stronger closer to the center, falls off
    // smoothstep creates a softer edge for the flare
    float flareIntensity = smoothstep(0.4, 0.05, distToFlare) * 0.55; // Adjust 0.4 (radius) and 0.55 (strength)
    
    vec3 flareColor = vec3(1.0, 0.95, 0.7) * flareIntensity; // Bright yellow-white flare

    // Combine cast color with flare using additive blending (screen-like)
...
const filterButtonData = [
  {
    name: "none",
    text: "Normal",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 18a6 6 0 0 0 0-12v12Z"/></svg>',
  },
  {
    name: "grayscale",
    text: "Grayscale",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 12h18"/></svg>',
  },
  {
    name: "sepia",
    text: "Sepia",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3c-1.93 0-3.68.73-4.95 1.95S4.27 7.07 4.27 9c0 1.2.38 2.31.97 3.26L3 17h18l-2.24-4.74c.59-.95.97-2.06.97-3.26 0-1.93-.73-3.68-1.95-4.95S13.93 3 12 3Z"/><path d="M12 12v1"/></svg>',
  },
  {
    name: "invert",
    text: "Invert",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16z"/><path d="M12 6v12"/></svg>',
  },
  {
    name: "cloudy",
    text: "Cloudy",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z"/><path d="M22 10a4.5 4.5 0 0 0-4.5-4.5M17.5 19H9"/></svg>',
  },
  {
    name: "moonlight",
    text: "Moonlight",
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/><path d="M19 3v4M21 5h-4"/></svg>',
  }, // Simple moon with a star/sparkle
  {
    name: "sunlight",
    text: "Sunlight",
...
async function setupCamera() {
  try {
    ...
    alert(
      "Could not access the camera. Please ensure permissions are granted and no other application is using it. Try refreshing the page or checking browser settings."
    );
    grantAccessBtn.textContent = "Camera Access Denied";
...
function adjustCanvasToVideo() {
  ...
  const maxHeight =
    window.innerHeight - (filterButtonsContainer.offsetHeight + 40); // +40 for some margin
...
function initializeWebGL() {
  gl = cameraCanvas.getContext("webgl", { premultipliedAlpha: false });
  if (!gl) {
    alert(
      "WebGL is not supported or disabled on your browser. Filters will not work."
    );
...
function createFilterButtons() {
  filterButtonsContainer.innerHTML = filterButtonData
    .map(
      (btn) => `
                <button data-filter="${btn.name}" class="${
        btn.name === currentFilterName ? "active" : ""
      }" title="Apply ${btn.text} filter">
                    ${btn.icon}
                    <span>${btn.text}</span>
                </button>
            `
...
function applyFilter(filterName) {
  ...
  if (
    ...
    currentShaderProgramInfo &&
    currentShaderProgramInfo.program
  )
    return;
  ...
  if (switchShaderProgram(fragmentShaders[filterName])) {
    ...
    const newActiveButton = filterButtonsContainer.querySelector(
      `button[data-filter="${filterName}"]`
    );
    if (newActiveButton) newActiveButton.classList.add("active");
    console.log(`Successfully applied filter: ${filterName}`);
  } else {
    console.error(
      `Failed to apply filter: ${filterName}. Attempting to revert to 'none'.`
    );
    if (currentFilterName !== "none") {
      if (switchShaderProgram(fragmentShaders["none"])) {
        currentFilterName = "none";
        const activeButton =
          filterButtonsContainer.querySelector("button.active");
        if (activeButton) activeButton.classList.remove("active");
        const noneButton = filterButtonsContainer.querySelector(
          `button[data-filter="none"]`
        );
        if (noneButton) noneButton.classList.add("active");
      } else {
        console.error(
          "CRITICAL: Failed to revert to 'none' filter. WebGL rendering may be broken."
        );
      }
    }
  }
...
function renderLoop() {
  ...
  gl.vertexAttribPointer(
    currentShaderProgramInfo.attribLocations.vertexPosition,
    2,
    gl.FLOAT,
    false,
    0,
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, texCoordBuffer);
  gl.vertexAttribPointer(
    currentShaderProgramInfo.attribLocations.textureCoord,
    2,
    gl.FLOAT,
    false,
    0,
