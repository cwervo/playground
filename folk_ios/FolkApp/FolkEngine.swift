...
public final class FolkEngine {
    ...
    public func dumpDatabase() -> String {
        return queue.sync {
            var lines = ["=== Active Claims ==="]
            for stmt in statements {
                lines.append("  - \(stmt.terms.map { $0.description }.joined(separator: " "))")
            }
            lines.append("=== Matches ===")
            for match in matches.values {
                lines.append("  - Match \(match.id.uuidString.prefix(6)): childCount=\(match.childStatements.count)")
            }
